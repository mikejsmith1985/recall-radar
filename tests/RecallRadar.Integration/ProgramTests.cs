...
{
    private const string ConnectionSettingKey = "ConnectionStrings:RecallRadar";

    [Fact]
    public async Task Health_ReportsHealthyWhenTheDatabaseIsReachable()
    {
        await using var factory = new WebApplicationFactory<Program>()
            .WithWebHostBuilder(builder => builder.UseSetting(ConnectionSettingKey, postgres.ConnectionString));
    ...
    public async Task Health_ReportsUnhealthyWhenTheDatabaseIsUnreachable()
    {
        ...
        await using var factory = new WebApplicationFactory<Program>()
            .WithWebHostBuilder(builder => builder.UseSetting(ConnectionSettingKey, unreachable));
        using var client = factory.CreateClient();
